...
{
    public class OrderStockFailWorker : BackgroundService
    {
        private readonly IConnection _connection;
        private readonly IServiceScopeFactory _scopeFactory;
        private readonly ILogger<OrderStockFailWorker> _logger;
        private readonly MessagingSettings _settings;

        private string ExchangeName => _settings.IncomingStock.Exchange;
        private string QueueName => _settings.IncomingStock.FailQueue;
        private string RoutingKey => _settings.IncomingStock.FailRoutingKey;

        private IChannel? _channel;
        private string? _consumerTag;

        public OrderStockFailWorker(
            IConnection connection,
            IServiceScopeFactory scopeFactory,
            ILogger<OrderStockFailWorker> logger,
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            try
            {
                ...
                _logger.LogInformation("OrderStockFail consumer listening on {QueueName}", QueueName);

                await Task.Delay(Timeout.Infinite, stoppingToken);
            }
            catch (OperationCanceledException) when (stoppingToken.IsCancellationRequested)
            {
                _logger.LogInformation("OrderStockFail consumer stopping (cancellation requested)");
            }
            finally
            {
                await CleanupAsync();
          ...
          CancellationToken stoppingToken)
        {
            try
            {
                ...
                var handler = scope.ServiceProvider.GetRequiredService<IOrderStockFailMessageHandler>();
                ...
                {
                    _logger.LogWarning("Invalid envelope for OrderStockFail message: {Payload}", payload);
                ...
                try
                {
                    ...
                    if (!handled)
                    {
                        _logger.LogWarning("OrderStockFail handler returned unsuccessful result for {EventId}", envelope.EventId);
                    }
                ...
                {
                    _logger.LogError("Error handling OrderStockFail event {EventId}, marking failed and requeueing", envelope.EventId);
            ...
            {
                _logger.LogError(ex, "Unhandled error while processing OrderStockFail message {DeliveryTag}", ea.DeliveryTag);
                try
                {
                    await channel.BasicNackAsync(ea.DeliveryTag, multiple: false, requeue: true, cancellationToken: stoppingToken);
                }
                ...
                {
                    _logger.LogWarning(nackEx, "Failed to nack OrderStockFail message {DeliveryTag}", ea.DeliveryTag);
                }
            }
        }
        ...
        private async Task CleanupAsync()
        {
            ...
            {
                _logger.LogError(ex, "Error cancelling OrderStockFail consumer on queue {QueueName}", QueueName);
            }
            ...
            {
                _logger.LogError(ex, "Error closing channel for OrderStockFail queue {QueueName}", QueueName);
            }
