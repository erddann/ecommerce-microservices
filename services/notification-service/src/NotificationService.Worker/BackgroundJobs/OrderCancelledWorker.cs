...
public class OrderCancelledWorker : BackgroundService
{
    ...
    private readonly ILogger<OrderCancelledWorker> _logger;
    private IChannel? _channel;
    private string? _consumerTag;

    public OrderCancelledWorker(
        RabbitMqConnectionProvider connectionProvider,
        IServiceProvider serviceProvider,
        RabbitMqOptions options,
        ISettings<NotificationSettings> settings,
        ILogger<OrderCancelledWorker> logger)
    {
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        try
        {
            ...
            var queueName = _options.OrderCancelledQueue;
            var routingKey = _options.OrderCancelledRoutingKey;
            _logger.LogInformation("Starting OrderCancelled consumer for queue {Queue}", queueName);
            ...
            _logger.LogInformation("OrderCancelled consumer subscribed with tag {ConsumerTag}", _consumerTag);
    ...
    private async Task HandleMessageAsync(BasicDeliverEventArgs args, string queueName, CancellationToken stoppingToken)
    {
        try
        {
            ...
            _logger.LogInformation("Received OrderCancelledEvent with delivery tag {Tag}", args.DeliveryTag);
            ...
            {
                _logger.LogInformation("OrderCancelled event {EventId} already processed", envelope.EventId);
        ...
        {
            _logger.LogError(ex, "Error while handling OrderCancelled message with delivery tag {Tag}", args.DeliveryTag);
    ...
    {
        _logger.LogInformation("Cleaning up OrderCancelled consumer");
        if (_channel == null)
        {
            return;
        }
		...
		{
			_logger.LogError(ex, "Error while cancelling consumer for {Queue}", _options.OrderCancelledQueue);
		}
		...
		{
			_logger.LogError(ex, "Error while closing channel for {Queue}", _options.OrderCancelledQueue);
		}

        _channel.Dispose();
        _channel = null;
        _consumerTag = null;
        _logger.LogInformation("OrderCancelled consumer cleanup completed");
    }
