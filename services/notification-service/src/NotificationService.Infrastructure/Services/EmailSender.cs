...
public class EmailSender : IChannelSender
{
    ...
    public EmailSender(HttpClient httpClient, NotificationSenderOptions options, ILogger<EmailSender> logger)
    {
        ...
        _retryPolicy = Policy<HttpResponseMessage>
            ...
            .WaitAndRetryAsync(3, retryAttempt => TimeSpan.FromSeconds(Math.Pow(2, retryAttempt)),
                ...
                {
                    _logger.LogWarning("Email send failed, retrying in {Timespan}. Attempt {RetryAttempt}", timespan, retryAttempt);
                });
    }
    ...
    public async Task<NotificationSendResult> SendAsync(RenderedNotification notification, CancellationToken cancellationToken = default)
    {
		...
		_logger.LogError("Email send failed with status {Status} and error {Error}", response.StatusCode, error);
